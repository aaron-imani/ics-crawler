...
def get_links_in_range_by_fileSize(file_path, min_ratio, max_ratio, output_file_path):
    return get_links_in_range(file_path, min_ratio, max_ratio, output_file_path, 'File_size')
...
def get_links_in_range(file_path, min_value, max_value, output_file_path, criterion):
    ...
    pattern = re.compile(r'File: (.+)\.html\n=*[\S\s]*?Number of Tokens: (\d+)[\S\s]*?Number of Characters: (\d+)[\S\s]*?Number of Paragraphs: (\d+)[\S\s]*?Information Ratio: ([\d.]*?File Size: ([\d.]+)')
    ...
    for link, tokens, characters, paragraphs, info_ratio, file_size in matches:
        ...
        if min_value <= value <= max_value:
            ...
            extracted_info.append(f"\nFile: {link}.html\n==============================\nNumber of Tokens: {tokens}\nNumber of Characters: {characters}\nNumber of Paragraphs: {paragraphs}\nInformation Ratio: {info_ratio}\File Size: {file_size}\n")
